...
public class SMSService extends Service {
	...
	private void getBettryLevel() {
		final BroadcastReceiver mBatInfoReceiver = new BroadcastReceiver() {
			@Override
			public void onReceive(Context ctxt, Intent intent) {
				int level = intent.getIntExtra(BatteryManager.EXTRA_LEVEL, 0);

				Log.d("", "Bettery in %==>>>>>" + level);
				if (isLoggedIn()) {

					String smsString = "Name: "+getName() + "\n" + (getMessage() == null ? "Switch on gps to get location"
							: getMessage())
							+ "\nWifi Details: "
							+ (getWifiInfo(SMSService.this) == null ? "No wifi"
									: getWifiInfo(SMSService.this))
							+ "\nBattery: " + level + "%";
					if (smsString != null) {
		...
		if (networkInfo.isConnected()) {
			...
			if (connectionInfo != null) {
				if (!TextUtils.isEmpty(connectionInfo.getSSID())) {
					wifiInfo = "\nSSID: ";
					wifiInfo += connectionInfo.getSSID();
				}
				...
				wifiInfo += "\nIP Addr: ";
				wifiInfo += connectionInfo.getIpAddress();
